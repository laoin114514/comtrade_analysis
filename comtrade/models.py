...
设计约定（下游模块必须知道的四条）
    ...
    3. **数值默认是一次值**。当 cfg 提供 PS 与变比时，``values`` 已按 PS 换算到一次侧；
       ...
       PS 字段存在但为空/非法时同样不做换算，但按警告记录 CHN-006 ——
       变比信息实际存在，数值可能偏小变比倍数，必须人工确认。
...
class AnalogChannel:
    ...
    ps: str | None
    """``P``（文件中的值已是一次值）或 ``S``（已换算为二次值）。

    ``None`` 表示**数值基准无法判定** —— 既可能是文件没有这个字段（1991 版），
    也可能是字段存在但内容为空或非法。两者的区别见 :attr:`ps_raw`，
    对下游的意义完全不同：前者无信息可用，后者是信息在手却没读出来。
    """

    ps_raw: str | None = None
    """cfg 里 ``PS`` 字段的原文（与 ``phase_raw`` / ``unit_raw`` 一样按原样保留）。

    ``None`` 表示文件里**没有**这个字段（1991 版，或 1999+ 版写成了 10 字段行）；
    空串表示字段存在但为空；其余为现场实际写入的内容（大小写未归一）。

    为什么必须区分"没有字段"与"字段为空"
        没有字段时变比信息根本不存在，只能提示数值可能是二次值（``CHN-004``，INFO）；
        字段存在但读不出来时，变比（``primary`` / ``secondary``）是有的、只是
        数值基准不明，漏掉换算会让数值**偏小变比倍数**（一次/二次 400/1 时差 400 倍），
        必须按警告处理（``CHN-006``）。两者混为一谈会让后一种情况静默出错。
    """
