...
class Code:
    ...
    CHN     通道识别与归一化
    TIM     时间轴生成
    CHK     结果自检（解析完成后对结果本身的检查）
    SYS     兜底异常（不应出现，出现即缺陷）
    ======  ==========================================

    错误码一旦发布不应修改含义（测试用例与界面提示都依赖它）。

    **所有在代码中出现的错误码都必须登记在本类中** —— 界面按码组织提示语，
    散落在别处的字面量会被漏掉。tests/test_diagnostics.py 有守卫测试。
    """
    ...
    COMPANION_UNREADABLE = "FIL-008"
    """.hdr/.inf 伴随文件存在但读取失败（不影响解析结果，但报告会缺该段信息）。"""
    ...
    CFG_ENCODING_FALLBACK = "CFG-ENC"
    """文件不是 UTF-8，已按兜底编码（cp1251 / gb18030 / latin-1）解码。"""

    CFG_OK = "CFG-OK"
    """配置文件解析完成（正常路径的 INFO 级标记，界面可用于表示"已识别"）。"""
    ...
    """检测到缺失的采样点（缺失值哨兵，或 ASCII 空字段），已标记为无效。

    两种来源都并入无效掩码：哨兵是"文件用特定数值表示没采到"，
    空字段是"文件直接没写这个数"。后者在解析期就是 NaN，
    必须一起并进 ``invalid_mask``，否则 ``has_invalid`` / ``invalid_count``
    会漏报，下游据它判断"该通道能否参与计算"就会被误导。
    """
    ...
    **仅提示，不改变换算行为**：仍按标准施加 a/b 换算。确认某文件确实直接存
    ...
    DAT_KEY_FIELD_MISSING = "DAT-013"
    """ASCII 行的采样号或采样时标为空/非法，该采样点无法定位，已跳过。

    这两列必须能转成整数，因此不能像模拟量那样用 NaN 表示"没采到"：
    NaN 经 ``astype(int64)`` 会被**静默**转成 -9223372036854775808，
    采样号随之失去意义（下游按采样号索引会算出无意义结果），
    而空值流进"首采样号"还会让记录数校验算出天文数字、误报文件被截断。
    """

    DAT_DIGITAL_PADDING = "DAT-DIG-PAD"
    """开关量打包的未使用位存在非 0 值（通常无害）。"""

    DAT_OK = "DAT-OK"
    """数据文件解析完成（正常路径的 INFO 级标记）。"""
    ...
    CHN_PS_UNREADABLE = "CHN-006"
    """PS 字段存在但为空或无法识别，无法判定数值是一次值还是二次值，未做一次值换算。

    与 CHN-004 的区别是**信息在不在文件里**：CHN-004 是"文件根本没有变比字段"
    （1991 版，无信息可用，只能如实提示）；本条是"变比信息在文件里、
    只是数值基准读不出来" —— 漏掉换算会让数值偏小变比倍数（如 CT 400/1 差 400 倍），
    而数值本身（二次值）看起来完全正常，因此等级为 WARNING 而不是 INFO。
    """
    ...
    TIM_TIMESTAMP_ZERO = "TIM-TS-ZERO"
    """采样时标全为 0：字段存在但无意义，未用于生成时间轴。"""

    TIM_TIMESTAMP_INVALID = "TIM-TS-INVALID"
    """所有采样时标均为缺失值哨兵，未用于生成时间轴。"""

    TIM_OK = "TIM-OK"
    """时间轴生成完成（正常路径的 INFO 级标记）。"""

    # --------------------------------------------------------------- 结果自检
    CHK_CHANNEL_ALL_INVALID = "CHK-001"
    """通道的全部采样点都无效（NaN），无法参与计算。"""

    CHK_CHANNEL_CONSTANT = "CHK-002"
    """通道数值恒定，可能是通道未接线，或比例系数 a/b 有误。"""

    CHK_THREE_PHASE_INCOMPLETE = "CHK-003"
    """三相电流或三相电压不完整，依赖三相量的故障判据无法执行。"""

    CHK_THREE_PHASE_MISSING = "CHK-004"
    """未识别到任何三相电流或三相电压通道，需人工指定映射。"""

    # ----------------------------------------------------------- 兜底异常
    SYS_UNEXPECTED_ERROR = "SYS-001"
    """解析过程中发生未预期的异常，已转为可控错误（不应出现，出现即缺陷）。"""
